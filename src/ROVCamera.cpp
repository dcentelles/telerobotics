...
bool ROVCamera::SendingCurrentImage() { return imgInBuffer; }
...
void ROVCamera::_WaitForNewOrders(int timeout) {
  ...
  if (device.WaitForFramesFromRxFifo(timeout)) {
    ...
    Log->warn(
        "RX: timeout when trying to receive new orders from the operator!");
  }
}
  ...
  if (txStateSet) {
    ...
    if (bytesLeft > 0) // == (ImgInBuffer == True)
    {
      ...
      txdlf->PayloadUpdated(TransportPDU::OverheadSize + txStateLength +
                            imgTrunkInfoLength + nextTrunkLength);
      Log->debug("TX: transmitting packet with the current state and an image "
                 "trunk (Seq: {}) (FS: {})",
...
void ROVCamera::_SetEndianess() { bigEndian = DataLinkFrame::IsBigEndian(); }
