...
HROVSettings::HROVSettings() { _Init(); }

// HROVSettings::HROVSettings(const merbots_whrov_msgs::hrov_settings::ConstPtr
// & cstate) {
//	_Init();
...
//{
//    merbots_whrov_msgs::hrov_settings::Ptr cstate(new
//    merbots_whrov_msgs::hrov_settings());
...
void HROVSettings::SetROIConf(uint16_t x0, uint16_t y0, uint16_t x1,
...
void HROVSettings::GetROIConf(int16_t &x0, int16_t &y0, int16_t &x1,
...
uint16_t HROVSettings::GetROIX0() {
  uint8_t *ptr = roi;
  uint16_t res;
...
uint16_t HROVSettings::GetROIY0() {
  uint8_t *ptr = roi + 1;
  uint16_t res;
...
uint16_t HROVSettings::GetROIX1() {
  uint8_t *ptr = roi + 2;
  uint16_t res;
...
uint16_t HROVSettings::GetROIY1() {
  uint8_t *ptr = roi + 3;
  uint16_t res;
...
uint8_t HROVSettings::GetROIShift() { return *roi_shift >> 5; }

void HROVSettings::SetImgSize(uint16_t size) {
  if (bigEndian) {
    *img_size = size;
  } else {
...
uint16_t HROVSettings::GetImgSize() {
  uint16_t res;
  if (bigEndian) {
    res = *img_size;
  } else {
...
void HROVSettings::SetImgResolution(uint16_t resolution) {
  *img_resolution |= resolution & 0x1f;
}

uint8_t HROVSettings::GetImgResolution() { return *img_resolution & 0x1f; }

void HROVSettings::SetMaxPacketLength(uint16_t mpl) {
  if (bigEndian) {
...
uint16_t HROVSettings::GetMaxPacketLength() {
  uint16_t res;
  if (bigEndian) {
